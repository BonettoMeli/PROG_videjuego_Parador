...
from Clases_POO import Juego, Inventario, Boton
...
def inventario_f():
    ...
    if inventario_abierto:
        pantalla.blit(inventario_img, (470,500))
...
inventario_img = pygame.image.load("visual/inventario.png")
inventario_img = pygame.transform.scale(inventario_img, (500, 400))
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN:
            ...
            if Boton_bolso.collidepoint(evento.pos) and pantalla_actual not in pantallas_ocultas:
                inventario.abierto = not inventario.abierto

            if inventario.abierto:
                for i in range(len(casillas)):
                    if casillas[i].collidepoint(evento.pos):
                        if i < len(inventario.objetos):
                            if objeto_seleccionado == inventario.objetos[i]:
                                objeto_seleccionado = None
                            else:
                                objeto_seleccionado = inventario.objetos[i]
